@if(is_default_lang())
    <div class="panel">
        <div class="panel-title"><strong>{{__('Horário de check-in/check-out')}}</strong></div>
        <div class="panel-body">
            <div class="form-group d-none">
                <label>{{__('Permitir reserva para o dia inteiro')}}</label>
                <br>
                <label>
                    <input type="checkbox" name="allow_full_day" @if($row->allow_full_day) checked @endif value="1"> {{__("Ativar reserva para o dia inteiro")}}
                </label>
                <div class="small">
                    {{__("Você pode reservar o quarto por dia inteiro")}} <br>
                    {{__("Ex: reserva de 22 a 23, então os dias 22 e 23 ficam completos e outras pessoas não podem reservar")}}
                </div>
            </div>
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group">
                        <label>{{__('Horário do check-in')}}</label>
                        <input type="text" value="{{$row->check_in_time}}" placeholder="{{__("Ex: 12:00AM")}}" name="check_in_time" class="form-control">
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="form-group">
                        <label>{{__('Horário do check-out')}}</label>
                        <input type="text" value="{{$row->check_out_time}}" placeholder="{{__("Ex: 11:00AM")}}" name="check_out_time" class="form-control">
                    </div>
                </div>
            </div>
            @if(is_default_lang())
                <div class="row">
                    <div class="col-lg-6">
                        <div class="form-group">
                            <label class="control-label">{{__("Mínimo de dias de antecedência")}}</label>
                            <input type="number" name="min_day_before_booking" class="form-control" value="{{$row->min_day_before_booking}}" placeholder="{{__("Ex: 3")}}">
                            <i>{{ __("Deixe em branco se não precisar usar opção de mínimo de dias") }}</i>
                        </div>
                    </div>
                    <div class="col-lg-6">
                        <div class="form-group">
                            <label class="control-label">{{__("Mínimo de dias de estadia")}}</label>
                            <input type="number" name="min_day_stays" class="form-control" value="{{$row->min_day_stays}}" placeholder="{{__("Ex: 2")}}">
                            <i>{{ __("Deixe em branco se não precisar definir mínimo de dias de estadia") }}</i>
                        </div>
                    </div>
                </div>
            @endif
    ...
    <div class="panel">
        <div class="panel-title"><strong>{{__("Preço")}}</strong></div>
        <div class="panel-body">
            @if(is_default_lang())
                <div class="row">
                    <div class="col-lg-6">
                        <div class="form-group">
                            ...
                            <input type="number" step="any" min="0" name="price" class="form-control" value="{{$row->price}}" placeholder="{{__("Preço do hotel")}}">
                        </div>
                    </div>
                    <div class="col-lg-6 d-none">
                        <div class="form-group">
                            <label class="control-label">{{__("Preço promocional")}}</label>
                            <input type="number" step="any" name="sale_price" class="form-control" value="{{$row->sale_price}}" placeholder="{{__("Preço promocional do hotel")}}">
                            <span><i>{{__("Se o preço normal for menor que o promocional, será exibido o preço normal")}}</i></span>
                        </div>
                    </div>
                </div>
            @endif
                    ...
                    <div class="item" data-number="__number__">
                        <div class="row">
                            <div class="col-md-5">
                                @if(!empty($languages) && setting_item('site_enable_multi_lang') && setting_item('site_locale'))
                                    @foreach($languages as $language)
                                        @php $key = setting_item('site_locale') != $language->locale ? "_".$language->locale : "" @endphp
                                        <div class="g-lang">
                                            <div class="title-lang">{{$language->name}}</div>
                                ...
                                <div class="col-md-5">
                                    @if(!empty($languages) && setting_item('site_enable_multi_lang') && setting_item('site_locale'))
                                        @foreach($languages as $language)
                                            ...
                                            <div class="g-lang">
                                                <div class="title-lang">{{$language->name}}</div>
                                                <input type="text" __name__="service_fee[__number__][name{{$key}}]" class="form-control" placeholder="{{__('Nome da taxa')}}">
                                                <input type="text" __name__="service_fee[__number__][desc{{$key}}]" class="form-control" placeholder="{{__('Descrição da taxa')}}">
                                            </div>
                                        @endforeach
                                    @else
