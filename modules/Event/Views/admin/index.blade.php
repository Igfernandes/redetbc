...
@section('content')
    <div class="container-fluid">
        <div class="d-flex justify-content-between mb20">
            <h1 class="title-bar">{{!empty($recovery) ? __('Recuperação') : __("All Events")}}</h1>
                        ...
                        @if($rows->total() > 0)
                            @foreach($rows as $row)
                                <tr class="{{$row->status}}">
                                    ...
                                    <td>
                                        @if(empty($recovery))
                                            <a href="{{route('event.admin.edit',['id'=>$row->id])}}" class="btn btn-primary btn-sm"><i class="fa fa-edit"></i> {{__('Editar')}}</a>
                                        @endif
                                    </td>
                                </tr>
                            @endforeach
