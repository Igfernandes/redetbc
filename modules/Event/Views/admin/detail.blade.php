    ...
    <form action="{{route('event.admin.store',['id'=>($row->id) ? $row->id : '-1','lang'=>request()->query('lang')])}}" method="post">
        ...
        <div class="container-fluid">
            <div class="d-flex justify-content-between mb20">
                <div class="">
                    <h1 class="title-bar">{{$row->id ? __('Editar: ').$row->title : __('Adicionar novo evento')}}</h1>
                    @if($row->slug)
                        <p class="item-url-demo">{{__("Link permanente")}}: {{ url('event' ) }}/<a href="#" class="open-edit-input" data-name="slug">{{$row->slug}}</a>
                        </p>
                    @endif
                </div>
                <div class="">
                    @if($row->slug)
                        <a class="btn btn-primary btn-sm" href="{{$row->getDetailUrl(request()->query('lang'))}}" target="_blank">{{__("Ver Evento")}}</a>
                    @endif
                </div>
            </div>
                    ...
                    <div class="col-md-3">
                        ...
                        @if(is_default_lang())
                            <div class="panel">
                                ...
                                <div class="panel-body">
                                    <div class="form-group">
                                        <label>{{__('Evento em Destaque')}}</label>
                                        <br>
                                        <label>
