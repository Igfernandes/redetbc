...
<div class="row">
    <div class="col-sm-4">
        ...
        <p class="form-group-desc">{{__('Altere as configurações do sistema de fornecedor')}}</p>
    </div>
    <div class="col-sm-8">
        <div class="panel">
            <div class="panel-body">
                @if(is_default_lang())
                    <div class="form-group">
                        <div class="form-controls">
                            <div class="form-group">
                                <label> <input type="checkbox" @if($settings['vendor_enable'] ?? '' == 1) checked @endif name="vendor_enable" value="1"> {{__("Habilitar Fornecedor?")}}</label>
                            </div>
                        </div>
                    </div>
                    <div class="form-group" data-condition="vendor_enable:is(1)">
                        ...
                        <div class="form-controls">
                            <select name="vendor_commission_type" class="form-control">
                                <option value="percent" {{($settings['vendor_commission_type'] ?? '') == 'percent' ? 'selected' : ''  }}>{{__('Percentual')}}</option>
                                <option value="amount" {{($settings['vendor_commission_type'] ?? '') == 'amount' ? 'selected' : ''  }}>{{__('Quantia')}}</option>
                            </select>
                        </div>
                    </div>
                    <div class="form-group" data-condition="vendor_enable:is(1)">
            ...
            <div class="panel-body">
                @if(is_default_lang())
                    <div class="form-group">
                        <div class="form-controls">
                            <div class="form-group">
                                <label> <input type="checkbox" @if($settings['vendor_auto_approved'] ?? '' == 1) checked @endif name="vendor_auto_approved" value="1"> {{__("Aprovação Automática de Fornecedor?")}}</label>
                            </div>
                        </div>
                    </div>
                    <div class="form-group">
                        <label>{{__('Função do Fornecedor')}}</label>
                        <div class="form-controls">
            ...
            <div class="panel-body">
                @if(is_default_lang())
                    <div class="form-group">
                        <div class="form-controls">
                            <div class="form-group">
                                <label> <input type="checkbox" @if($settings['vendor_show_email'] ?? '' == 1) checked @endif name="vendor_show_email" value="1"> {{__("Mostrar email do fornecedor no perfil?")}}</label>
                            </div>
                        </div>
                    </div>
                    <div class="form-group">
                        <div class="form-controls">
                            <div class="form-group">
                                <label> <input type="checkbox" @if($settings['vendor_show_phone'] ?? '' == 1) checked @endif name="vendor_show_phone" value="1"> {{__("Mostrar telefone do fornecedor no perfil?")}}</label>
                            </div>
                        </div>
                    </div>
                @else
            ...
            <div class="panel-body">
                ...
                <div class="form-group">
                    <div class="form-controls">
                        ...
                        <div class="form-group">
                            <label> <input type="checkbox" @if(setting_item('disable_payout') == 1) checked @endif name="disable_payout" value="1"> {{__("Sim, por favor desativar")}}</label>
                        </div>
                    </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    <div class="form-controls">
                        <div><label><input name="vendor_payout_booking_status[]" @if(in_array('processing',$vendor_payout_booking_status)) checked @endif type="checkbox" value="processing"> {{__("Processando")}}</label></div>
                        <div><label><input  name="vendor_payout_booking_status[]" @if(in_array('confirmed',$vendor_payout_booking_status)) checked @endif type="checkbox" value="confirmed"> {{__("Confirmado")}}</label></div>
                        <div><label><input  name="vendor_payout_booking_status[]" @if(in_array('completed',$vendor_payout_booking_status)) checked @endif type="checkbox" value="completed"> {{__("Concluído")}}</label></div>
                ...
                <div class="form-group">
                    <label><strong>{{__("Métodos de Pagamento")}}</strong></label>
                    <div class="form-controls">
                            ...
                            <div class="form-group-item">
                                ...
                                <div class="g-items">
                                    ...
                                    @foreach($items as $key=>$item)
                                        <div class="item" data-number="{{$key}}">
                                            <div class="row">
                                                <div class="col-md-3">
                                                    <input placeholder="{{__('Ex: bank_transfer')}}" type="text" name="vendor_payout_methods[{{$key}}][id]" class="form-control" value="{{$item->id}}" >
                                                </div>
                                                <div class="col-md-6">
                                                    <label >{{__("Nome")}}</label>
                                ...
                                <div class="g-more hide">
                                    <div class="item" data-number="__number__">
                                        <div class="row">
                                            <div class="col-md-3">
                                                <input placeholder="{{__('Ex: bank_transfer')}}" type="text" __name__="vendor_payout_methods[__number__][id]" class="form-control" value="" >
                                            </div>
                                            <div class="col-md-6">
                                                <label >{{__("Nome")}}</label>
...
<div class="row">
    <div class="col-sm-4">
        <h3 class="form-group-title">{{__('Conteúdo do Email para o Fornecedor Registrado')}}</h3>
        <div class="form-group-desc">{{ __('Conteúdo do email enviado para o Fornecedor ou Administrador quando um usuário se registra.')}}
    ...
    <div class="col-sm-8">
        <div class="panel">
            <div class="panel-body">
                @if(is_default_lang())
                    <div class="form-group">
                        <label> <input type="checkbox" @if($settings['enable_mail_vendor_registered'] ?? '' == 1) checked @endif name="enable_mail_vendor_registered" value="1"> {{__("Habilitar envio de email para o cliente quando o cliente se registrar ?")}}</label>
                    </div>
                @else
                    <div class="form-group">
                        <label> <input type="checkbox" @if($settings['enable_mail_vendor_registered'] ?? '' == 1) checked @endif disabled name="enable_mail_vendor_registered" value="1"> {{__("Habilitar envio de email para o cliente quando o cliente se registrar ?")}}</label>
                    </div>
                ...
                <div class="form-group" data-condition="enable_mail_vendor_registered:is(1)">
                    <label>{{__("Email para o Fornecedor")}}</label>
                    <div class="form-controls">
                ...
                @if(is_default_lang())
                    <div class="form-group">
                        <label> <input type="checkbox" @if($settings['admin_enable_mail_vendor_registered'] ?? '' == 1) checked @endif name="admin_enable_mail_vendor_registered" value="1"> {{__("Habilitar envio de email para o Administrador quando o cliente se registrar ?")}}</label>
                    </div>
                @else
                    <div class="form-group">
                        <label> <input type="checkbox" @if($settings['admin_enable_mail_vendor_registered'] ?? '' == 1) checked @endif disabled name="admin_enable_mail_vendor_registered" value="1"> {{__("Habilitar envio de email para o Administrador quando o cliente se registrar ?")}}</label>
                    </div>
