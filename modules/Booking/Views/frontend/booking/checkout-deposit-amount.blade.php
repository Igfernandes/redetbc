@if(auth()->check() && $booking->status == 'draft' && empty(setting_item('wallet_module_disable')))
    ...
    <div class="form-group-item">
        <h5 class="form-section-title">{{__("Quer pagar crédito?")}}</h5>
        ...
        <div class="d-flex justify-content-between"><h5 class="form-section-title">{{__("Pague agora")}}:</h5> <div class="val convert_pay_now">{{format_money(floatval($booking->deposit == null ? $booking->total : $booking->deposit))}}</div></div>
    </div>
@endif
