...
class AssistanceController extends AdminController
{
    ...
    protected $assistance_translation;
    protected $assistance_term;
    protected $attributes;
    protected $location;

    public function __construct(Assistance $assistance,AssistanceTranslation $assistance_translation,AssistanceTerm $assistance_term,Attributes $attributes, Location $location)
    {
        $this->setActiveMenu(route('assistance.admin.index'));
        $this->assistance = $assistance;
        $this->assistance_translation = $assistance_translation;
        $this->assistance_term = $assistance_term;
    ...
    public function index(Request $request)
    {
        ...
        $data = [
            ...
            'page_title'        => __("Assistance Management")
        ];
    ...
    {
        $this->checkPermission('assistance_view');
        $query = $this->assistance::onlyTrashed();
        $query->orderBy('id', 'desc');
        if (!empty($s = $request->input('s'))) {
            $query->where('title', 'LIKE', '%' . $s . '%');
            $query->orderBy('title', 'asc');
        ...
        $data = [
            ...
            'page_title'        => __("Recovery Assistance Management")
        ];
        return view('Assistance::admin.index', $data);
    }
    ...
    public function create(Request $request)
    {
        ...
        $row = new $this->assistance();
        $row->fill([
            'status' => 'publish'
        ]);
        $data = [
            'row'          => $row,
            'attributes'   => $this->attributes::where('service', 'assistance')->get(),
            'assistance_location' => $this->location::where('status', 'publish')->get()->toTree(),
    ...
    public function store(Request $request, $id)
    {
        ...
        if ($res) {
            ...
            if ($id > 0) {
                ...
                return redirect(route('assistance.admin.edit', $row->id))->with('success', __('Service created'));
            }
        }
    }
    ...
    public function saveTerms($row, $request)
    {
        ...
        if (empty($request->input('terms'))) {
            $this->assistance_term::where('target_id', $row->id)->delete();
        } else {
            $term_ids = $request->input('terms');
            foreach ($term_ids as $term_id) {
                $this->assistance_term::firstOrCreate([
                    'term_id'   => $term_id,
                    'target_id' => $row->id
                ]);
            }
            $this->assistance_term::where('target_id', $row->id)->whereNotIn('term_id', $term_ids)->delete();
        }
    }
    ...
    public function bulkEdit(Request $request)
    {
        ...
        switch ($action) {
            case "delete":
                foreach ($ids as $id) {
                    $query = $this->assistance::where("id", $id);
                    if (!$this->hasPermission('assistance_manage_others')) {
            ...
            case "recovery":
                foreach ($ids as $id) {
                    $query = $this->assistance::withTrashed()->where("id", $id);
                    if (!$this->hasPermission('assistance_manage_others')) {
                        $query->where("create_user", Auth::id());
                        $this->checkPermission('assistance_delete');
                    }
    ...
    public function getForSelect2(Request $request)
    {
        ...
        if ($pre_selected && $selected) {
            if (is_array($selected)) {
                $items = $this->assistance::select('id', 'title as text')->whereIn('id', $selected)->take(50)->get();
            } else {
                $items = $this->assistance::find($selected);
            }
            return [
                'results'=>$items
            ];
        ...
        $q = $request->query('q');
        $query = $this->assistance::select('id', 'title as text')->where("status", "publish");
        if ($q) {
