...
@section('content')
    <div class="container-fluid">
        <div class="d-flex justify-content-between mb20">
            <h1 class="title-bar">{{!empty($recovery) ? __('Recuperação') : __("Todos Serviços")}}</h1>
        ...
        <div class="filter-div d-flex justify-content-between ">
            <div class="col-left">
                @if(!empty($rows))
                    <form method="post" action="{{route('assistance.admin.bulkEdit')}}" class="filter-form filter-form-left d-flex justify-content-start">
                        {{csrf_field()}}
                    ...
                    @if(!empty($rows) and $assistance_manage_others)
                        <div class="ml-3 position-relative">
                            ...
                            <div class="dropdown-menu px-3 py-3 dropdown-menu-right" aria-labelledby="dropdown_filters">
                                <div class="mb-3">
                                    <label class="d-block" for="exampleInputEmail1">{{ __("Categoria") }}</label>
                                    <select name="cate_id" class="form-control">
                                        <option value="">{{ __('-- Todas Categorias --')}} </option>
                                        @foreach($assistance_categories as $category)
                            ...
                            @if($rows->total() > 0)
                                @foreach($rows as $row)
                                    <tr class="{{$row->status}}">
                                        ...
                                        <td class="title">
                                            <a href="{{route('assistance.admin.edit',['id'=>$row->id])}}">{{$row->title}}</a>

                                            @if($row->is_featured)
                                                <span class="badge badge-warning">{{ __("Destaque") }}</span>
                                            @endif
                                        </td>
