...
@section('content')
    <div class="container-fluid">
        <div class="d-flex justify-content-between mb20">
            <h1 class="title-bar">{{__("Atributos de serviço")}}</h1>
        </div>
        ...
        <div class="row">
            ...
            <div class="col-md-8">
                <div class="filter-div d-flex justify-content-between ">
                    <div class="col-left">
                        @if(!empty($rows))
                            <form method="post" action="{{route('assistance.admin.attribute.editAttrBulk')}}" class="filter-form filter-form-left d-flex justify-content-start">
                                {{csrf_field()}}
                    ...
                    <div class="col-left">
                        <form method="get" action="{{route('assistance.admin.attribute.index')}} " class="filter-form filter-form-right d-flex justify-content-end" role="search">
                ...
                <div class="panel">
                    <div class="panel-title">{{__("Todos os atributos")}}</div>
                    <div class="panel-body">
                                ...
                                @if(count($rows) > 0)
                                    @foreach($rows as $row)
                                        <tr>
                                            ...
                                            <td class="title">
                                                <a href="{{route('assistance.admin.attribute.edit',['id'=>$row->id])}}">{{$row->name}}</a>
                                            </td>
                                            <td>
                                                {{$row->position ?? 0}}
                                            </td>
                                            <td>
                                                <a href="{{route('assistance.admin.attribute.edit',['id'=>$row->id])}}" class="btn btn-primary btn-sm"><i class="fa fa-edit"></i> {{__('Editar')}}
                                                </a>
                                                <a href="{{route('assistance.admin.attribute.term.index',['id'=>$row->id])}}" class="btn btn-sm btn-success"><i class="fa fa"></i> {{__("Gerenciar Termos")}}
                                                </a>
