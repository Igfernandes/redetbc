<div class="panel">
    ...
    <div class="panel-body">
        @if(is_default_lang())
            <h3 class="panel-body-title">{{__("Preço do Serviço")}}</h3>
            <div class="row">
                <div class="col-lg-6">
                    <div class="form-group">
                        <label class="control-label">{{__("Preço")}}</label>
                        <input type="text" name="price" class="form-control" value="{{old('price',$row->price)}}" placeholder="{{__("Preço do Serviço")}}">
                    </div>
                </div>
                <div class="col-lg-6">
                    <div class="form-group">
                ...
                <div class="col-lg-12">
                    <span>
                        {{__("Se o preço regular for menor que o preço com desconto, será exibido apenas o preço regular.")}}
                    </span>
                </div>
            </div>
            <hr>
                                        ...
                                        @if(!empty($languages) && setting_item('site_enable_multi_lang') && setting_item('site_locale'))
                                            @foreach($languages as $language)
                                                ...
                                                <div class="g-lang">
                                                    ...
                                                    <input type="text" name="person_types[{{$key}}][name{{$key_lang}}]" class="form-control" value="{{$person_type['name'.$key_lang] ?? ''}}" placeholder="{{__('Ex: Adultos')}}">
                                                    <input type="text" name="person_types[{{$key}}][desc{{$key_lang}}]" class="form-control" value="{{$person_type['desc'.$key_lang] ?? ''}}" placeholder="{{__('Descrição')}}">
                                                </div>
                                            @endforeach
                                        @else
                                            <input type="text" name="person_types[{{$key}}][name]" class="form-control" value="{{$person_type['name'] ?? ''}}" placeholder="{{__('Ex: Adultos')}}">
                                            <input type="text" name="person_types[{{$key}}][desc]" class="form-control" value="{{$person_type['desc'] ?? ''}}" placeholder="{{__('Descrição')}}">
                                        @endif
                                    </div>

                                    <div class="col-md-2">
                                        <input type="number" min="0" name="person_types[{{$key}}][min]" class="form-control" value="{{$person_type['min'] ?? 0}}" placeholder="{{__("Mínimo por reserva")}}">
                                    </div>

                                    <div class="col-md-2">
                                        <input type="number" min="0" name="person_types[{{$key}}][max]" class="form-control" value="{{$person_type['max'] ?? 0}}" placeholder="{{__("Máximo por reserva")}}">
                                    </div>

                                    <div class="col-md-2">
                                        <input type="text" min="0" name="person_types[{{$key}}][price]" class="form-control" value="{{$person_type['price'] ?? 0}}" placeholder="{{__("por 1 unidade")}}">
                                    </div>
                            ...
                            <div class="col-md-5">
                                @if(!empty($languages) && setting_item('site_enable_multi_lang') && setting_item('site_locale'))
                                    @foreach($languages as $language)
                                        ...
                                        <div class="g-lang">
                                            <div class="title-lang">{{$language->name}}</div>
                                            <input type="text" __name__="person_types[__number__][name{{$key}}]" class="form-control" placeholder="{{__('Ex: Adultos')}}">
                                            <input type="text" __name__="person_types[__number__][desc{{$key}}]" class="form-control" placeholder="{{__('Descrição')}}">
                                        </div>
                                    @endforeach
                                @else
                                    <input type="text" __name__="person_types[__number__][name]" class="form-control" placeholder="{{__('Ex: Adultos')}}">
                                    <input type="text" __name__="person_types[__number__][desc]" class="form-control" placeholder="{{__('Descrição')}}">
                                @endif
                            </div>

                            <div class="col-md-2">
                                <input type="number" min="0" __name__="person_types[__number__][min]" class="form-control" placeholder="{{__("Mínimo por reserva")}}">
                            </div>

                            <div class="col-md-2">
                                <input type="number" min="0" __name__="person_types[__number__][max]" class="form-control" placeholder="{{__("Máximo por reserva")}}">
                            </div>

                            <div class="col-md-2">
                                <input type="text" min="0" __name__="person_types[__number__][price]" class="form-control" placeholder="{{__("por 1 unidade")}}">
                            </div>
        ...
        @if(is_default_lang())
            <hr>
            <h3 class="panel-body-title">{{__('Preço Extra')}}</h3>

            <div class="form-group">
                <label><input type="checkbox" name="enable_extra_price" @if(!empty($row->meta->enable_extra_price)) checked @endif value="1"> {{__('Ativar preço extra')}}</label>
            </div>
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-3">
                                        <select name="extra_price[{{$key}}][type]" class="form-control">
                                            <option @if($extra_price['type'] == 'one_time') selected @endif value="one_time">{{__("Único")}}</option>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-3">
                                <select __name__="extra_price[__number__][type]" class="form-control">
                                    <option value="one_time">{{__("Único")}}</option>
        ...
        @if(is_default_lang())
            <hr>
            <h3 class="panel-body-title">{{__('Desconto por quantidade de pessoas')}}</h3>

            <div class="form-group-item">
                <div class="g-items-header">
                    <div class="row">
                        <div class="col-md-4">{{__("Nº de pessoas")}}</div>
                        <div class="col-md-3">{{__('Desconto')}}</div>
                        <div class="col-md-3">{{__('Tipo')}}</div>
                        <div class="col-md-1"></div>
                    </div>
                </div>
                        ...
                        @foreach($discount_by_people as $key=>$item)
                            <div class="item" data-number="{{$key}}">
                                <div class="row">
                                    ...
                                    <div class="col-md-2">
                                        <input type="number" min="0" name="discount_by_people[{{$key}}][from]" class="form-control" value="{{$item['from']}}" placeholder="{{__('De')}}">
                                    </div>

                                    <div class="col-md-2">
                                        <input type="number" min="0" name="discount_by_people[{{$key}}][to]" class="form-control" value="{{$item['to']}}" placeholder="{{__('Até')}}">
                                    </div>
                                    ...
                                    <div class="col-md-3">
                                        <select name="discount_by_people[{{$key}}][type]" class="form-control">
                                            <option @if($item['type'] == 'fixed') selected @endif value="fixed">{{__("Fixo")}}</option>
                                            <option @if($item['type'] == 'percent') selected @endif value="percent">{{__("Percentual (%)")}}</option>
                                        </select>
                                    </div>
                ...
                <div class="g-more hide">
                    <div class="item" data-number="__number__">
                        <div class="row">
                            ...
                            <div class="col-md-2">
                                <input type="number" min="0" __name__="discount_by_people[__number__][from]" class="form-control" placeholder="{{__('De')}}">
                            </div>

                            <div class="col-md-2">
                                <input type="number" min="0" __name__="discount_by_people[__number__][to]" class="form-control" placeholder="{{__('Até')}}">
                            </div>
        ...
        @if(is_default_lang() and (!empty(setting_item("assistance_allow_vendor_can_add_service_fee")) or is_admin()))
            <hr>
            <h3 class="panel-body-title">{{ __('Taxas de Serviço')}}</h3>

            <div class="form-group">
                <label><input type="checkbox" name="enable_service_fee" @if(!empty($row->enable_service_fee)) checked @endif value="1"> {{__('Ativar taxa de serviço')}}</label>
            </div>
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-3">
                                        ...
                                        <select name="service_fee[{{$key}}][unit]" class="form-control">
                                            ...
                                            <option @if(($item['unit'] ?? "") == 'percent') selected @endif value="percent">{{ __("Percentual") }}</option>
                                        </select>
                                    </div>

                                    <div class="col-md-3">
                                        <label>
                                            <input type="checkbox" name="service_fee[{{$key}}][per_person]" value="on" @if($item['per_person'] ?? '') checked @endif>
                                            {{__("Preço por pessoa")}}
                                        </label>
                                    </div>
                ...
                <div class="g-more hide">
                    <div class="item" data-number="__number__">
                        <div class="row">
                            ...
                            <div class="col-md-5">
                                <input type="text" __name__="service_fee[__number__][name]" class="form-control" placeholder="{{__('Nome da taxa')}}">
                                <input type="text" __name__="service_fee[__number__][desc]" class="form-control" placeholder="{{__('Descrição da taxa')}}">
                            </div>
