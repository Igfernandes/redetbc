...
jQuery(function ($) {
    ...
    $(".upload-btn-wrapper").each(function () {
        ...
        container
            .find(".btn-file :file")
            .on("bravo-file-update-success", function (event, data) {
                console.log(data);
                container
                ...
                container
                    .find(".image-demo")
                    .attr("src", data.data.sizes.default);
                container
                    .find(".text-view")
                    .attr("value", data.data.sizes.default);
            });
        container
            .find(".btn-file :file")
            .on("bravo-file-before-update", function () {
                container
                    .find(".text-view")
                    .attr(
                        "value",
                        container.find(".text-view").data("loading")
                    );
            });
        container
            .find(".btn-file :file")
            .on("bravo-file-update-error", function (event, message) {
                if (message.length > 0) {
                    ...
                    container
                        ...
                        .attr(
                            "value",
                            container.find(".text-view").data("error")
                        );
                }
            });
    });
    ...
    $(document).on(
        "click",
        ".dungdt-upload-box-normal .edit-img, .dungdt-upload-multiple .edit-img, .show_avatar .edit-img",
        function (e) {
            e.preventDefault();
            let $this = $(this);
            let image_path = $this.attr("data-file");
            let edit_type = $this.hasClass("edit-multiple")
                ? "multiple"
                : "single";
            let p =
                edit_type === "multiple"
                    ? $this.closest(".dungdt-upload-multiple")
                    : $this.closest(".dungdt-upload-box");
            let image_id =
                edit_type === "multiple"
                    ? $this.attr("data-id")
                    : p.attr("data-val");
            ...
            let callback = {
                ...
                onComplete: function (url) {
                    ...
                    if (edit_type === "multiple") {
                        ...
                        p.find(".attach-demo").html(
                            '<img src="' +
                                canvas +
                                '" alt="image-responsive" style="max-width: 150px">'
                        );
                    }
    ...
    $(document).on(
        "click",
        ".dungdt-upload-box-normal .btn-field-upload,.dungdt-upload-box-normal .attach-demo",
        function () {
            ...
            uploaderModal.show({
                ...
                onSelect: function (files) {
                    let path =
                        files[0].edit_path !== undefined
                            ? files[0].edit_path
                            : files[0].max_large_size;
                    p.addClass("active");
                    p.find(".attach-demo").html(
                        '<img src="' + files[0].thumb_size + '"/>'
                    );
                ...
                onSelect: function (files) {
                    if (typeof files != "undefined" && files.length) {
                        ...
                        for (var i = 0; i < files.length; i++) {
                            let path =
                                files[i].edit_path !== undefined
                                    ? files[i].edit_path
                                    : files[i].max_large_size;
                            ids.push(files[i].id);
                            html +=
                                '<div class="image-item">' +
                                '<div class="inner">';
                            html +=
                                '<a class="edit-img btn btn-sm btn-primary edit-multiple" data-id="' +
                                files[i].id +
                                '" data-file="' +
                                path +
                                '"><i class="fa fa-edit"></i></a>';
                            html +=
                                '<span class="delete btn btn-sm btn-danger"><i class="fa fa-trash"></i></span><div class="img-preview"><img class="image-responsive image-preview w-100" src="' +
                                files[i].thumb_size +
                                '"/></div>' +
                                "</div>" +
                                "</div>";
    ...
    $(".bravo_user_profile .bravo-list-item .control-action .btn-danger").click(
        function () {
    ...
    $(
        ".bravo_user_profile .bravo-list-item .control-action .btn-recovery"
    ).click(function () {
        var c = confirm($(this).data("confirm"));
        if (!c) {
            return false;
    ...
    $(".has-ckeditor").each(function () {
        ...
        tinymce.init({
            selector: "#" + id,
            plugins:
                "preview searchreplace autolink code fullscreen image link media codesample table charmap hr toc advlist lists wordcount imagetools textpattern help pagebreak hr",
            toolbar:
        ...
        for (var i = 0; i < me.get(0).files.length; i++) {
            ...
            $.ajax({
                ...
                success: function (res) {
                    ...
                    father.classList.remove("loading");
                },
                error: function (e) {
