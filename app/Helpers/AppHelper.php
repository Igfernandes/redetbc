...
function hasRequestAffiliate()
{
    if (empty(Auth::user()))
        return false;

    $foundUpgradeRequest = VendorRequest::where([
        "user_id" => Auth::user()->id,
        "status" =>  "pending",
        "type" => "affiliate"
    ])->first();

    if (!empty($foundUpgradeRequest))
        return true;

    return;
}

function hasUpgradePlanRequest()
{
    return false;

    $foundUpgradeRequest = VendorRequest::where("user_id", Auth::user()->id)->first();
    $role = Role::where("name", "Traveler")->first();

    if (!$foundUpgradeRequest) return;

    if ($foundUpgradeRequest->status === "approved" && Auth::user()->role_id == $role->id)
        return true;
