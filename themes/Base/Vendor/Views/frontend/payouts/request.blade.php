...
@if($available_payout_amount)
    ...
    <br>
    <div class="">
        <a href="#vendor_create_request" data-toggle="modal" class="btn btn-success btn-sm"><i class="fa fa-plus"></i> {{__("Criar solicitação")}}</a>
    </div>
@else
    <div class="alert alert-warning">{{__("Seu saldo é zero")}}</div>
@endif
    ...
    <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">{{__("Criar solicitação de pagamento")}}</h5>
            ...
            <div class="modal-body ">
                <form class="" novalidate onsubmit="return false" >
                    <div class="form-group row">
                        <label class="col-sm-3 col-form-label">{{__("Disponível para pagamento")}}</label>
                        <div class="col-sm-9">
                    ...
                    <div class="form-group row">
                        <label class="col-sm-3 col-form-label">{{__("Método")}} <span class="text-danger">*</span></label>
                        <div class="col-sm-9">
                            <select required class="form-control" name="payout_method">
                                <option value="">{{__('-- Selecione --')}}</option>
                                @foreach($currentUser->available_payout_methods as $id=>$method)
                                    <option value="{{$id}}">{{$method->name}} @if(!empty($method->min)) ({{__('Mínimo: :amount',['amount'=>format_money($method->min)])}}) @endif</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    <div class="form-group row">
                        <label class="col-sm-3 col-form-label">{{__("Nota para o administrador")}}</label>
                        <div class="col-sm-9">
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-success " onclick="vendorPayout.sendRequest(this)">{{__('Enviar solicitação')}}
