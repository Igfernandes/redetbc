            ...
            <div class="card-header card-collapse bg-transparent border-0">
                <h5 class="mb-0">
                    <button type="button"
                        ...
                        data-target="#basicsCollapseDetail">
                        ...
                        @if(!empty($person_types))
                            @foreach($person_types as $type)
                                <li class="d-flex justify-content-between py-2">
                                    ...
                                    <div class="val">{{ $type['number'] }}</div>
                                </li>
                            @endforeach
                        @else
                            <li class="d-flex justify-content-between py-2">
                                <div class="label">{{ __("Convidados") }}:</div>
                                <div class="val">{{ $booking->total_guests }}</div>
                            </li>
                        @endif
            ...
            <div class="card-header card-collapse bg-transparent border-0">
                <h5 class="mb-0">
                    <button type="button"
                        class="btn btn-link border-0 btn-block d-flex justify-content-between card-btn py-3 px-4 font-size-17 font-weight-bold text-dark"
                        data-toggle="collapse"
                        data-target="#basicsCollapsePayment">
            ...
            <div id="basicsCollapsePayment" class="collapse show">
                <div class="card-body px-4 pt-0">
                    ...
                    <ul class="list-unstyled font-size-1 mb-0 font-size-16">
                        ...
                        {{-- Preços por tipo de pessoa --}}
                        @if(!empty($person_types))
                            @foreach($person_types as $type)
                                <li class="d-flex justify-content-between py-2">
                                    <div class="label">
                                        {{ $type['name_'.$lang_local] ?? __($type['name']) }}:
                                        {{$type['number']}} × {{ format_money($type['price']) }}
                                    </div>
                                    <div class="val">
                        ...
                        @else
                            ...
                            <li class="d-flex justify-content-between py-2">
                                <div class="label">
                                    {{ __("Convidados") }}:
                                    {{$booking->total_guests}} × {{ format_money($booking->getMeta('base_price')) }}
                                </div>
                                <div class="val">
                        ...
                        @if(!empty($extra_price))
                            <li class="d-flex justify-content-between py-2">
                                <div class="font-size-16 font-weight-bold">{{ __("Preços Extras:") }}</div>
                            </li>

                            @foreach($extra_price as $type)
                                <li class="d-flex justify-content-between py-2">
                                    <div class="label">{{ $type['name_'.$lang_local] ?? __($type['name']) }}:</div>
                                    <div class="val">{{ format_money($type['total'] ?? 0) }}</div>
                                </li>
                            @endforeach
                        @endif
                        ...
                        @if(!empty($discount_by_people))
                            <li class="d-flex justify-content-between py-2">
                                <div class="font-size-16 font-weight-bold">{{ __("Descontos:") }}</div>
                            </li>
                        ...
                        @if(!empty($list_all_fee))
                            @foreach ($list_all_fee as $item)
                                ...
                                <li class="d-flex justify-content-between py-2">
                                    <div class="font-size-16 font-weight-bold">{{ __("Taxa:") }}</div>
                                </li>
                                ...
                                <li class="d-flex justify-content-between py-2">
                                    <div class="label">
                                        ...
                                        @if(!empty($item['per_person']) && $item['per_person'] == "on")
                        ...
                        @if($booking->status != 'draft')
                            ...
                            @if($booking->paid < $booking->total)
                                <li class="d-flex justify-content-between py-2">
                                    <div class="label">{{ __("Pendente:") }}</div>
