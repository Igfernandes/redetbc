...
@endpush
@section('content')
    <div class="bravo_search_tour bravo_search_assistance">
        <h1 class="d-none">
            {{setting_item_with_lang("assistance_page_search_title")}}
        </h1>
        <div class="bravo_form_search_map">
            @include('assistance::frontend.layouts.search-map.form-search-map')
        </div>
        <div class="bravo_search_map {{ setting_item_with_lang("assistance_layout_map_option",false,"map_left") }}">
            ...
            <div class="results_item">
                @include('assistance::frontend.layouts.search-map.advance-filter')
                <div class="listing_items ajax-search-result">
                    @include('assistance::frontend.ajax.search-result-map')
                </div>
            </div>
        </div>
    </div>
...
@push('js')
    ...
    <script>
        var bravo_map_data = {
            ...
            map_lat_default:{{setting_item('assistance_map_lat_default','0')}},
            map_lng_default:{{setting_item('assistance_map_lng_default','0')}},
            map_zoom_default:{{setting_item('assistance_map_zoom_default','6')}},
        };
    </script>
