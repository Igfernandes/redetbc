    ...
    <div class="col dropdown-custom px-0 mb-4 form-select-guests">
        ...
        <div class="flex-horizontal-center border-bottom border-width-2 border-color-1 py-2 d-flex  dropdown-toggle" data-toggle="dropdown">
            ...
            <div class="text-black font-size-16 font-weight-semi-bold mr-auto">
               <div class="render">
                    <span class="adults" ><span class="one @if($adults >1) d-none @endif">{{__('1 Adulto')}}</span> <span class="@if($adults <= 1) d-none @endif multi" data-html="{{__(':count Adulto')}}">{{__(':count Adulto',['count'=>request()->query('adults',1)])}}</span></span>
                    -
                    <span class="children" >
                        ...
                        <span class="multi @if($children <=1) d-none @endif" data-html="{{__(':count criança')}}">{{__(':count criança',['count'=>request()->query('children',0)])}}</span>
                    </span>
               </div>
            </div>
        </div>
